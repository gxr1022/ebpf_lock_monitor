...
BPF_HASH(tracing, u32 __user *, struct contention,1048576); 
BPF_HASH(start, u32, struct futex_lock,1048576); // pid -> futex call start timestamp

// Mutex to the stack id which initialized that mutex
BPF_HASH(init_stacks, u64, struct mutex_lock_init,1048576);
// Main info database about mutex and thread pairs
BPF_HASH(locks, struct thread_mutex_key_t, struct thread_mutex_val_t,1048576);
// Pid to the mutex address and timestamp of when the wait started
BPF_HASH(lock_start, u32, struct mutex_timestamp_t,1048576);
// Pid and mutex address to the timestamp of when the wait ended (mutex acquired) and the stack id
BPF_HASH(lock_end, struct mutex_lock_time_key_t, struct mutex_lock_time_val_t, 1048576);
